...
def build_llm_prompt(stats: SummaryStats, config: AppConfig) -> str:
    ...
    payload = {
        ...
        "告警条数统计": {
            "未处理_非带宽告警": len(det_unhandled),
            "已结束_非带宽告警": len(det_ended),
        },
    }
    payload_json = json.dumps(payload, ensure_ascii=False, indent=2)
    return f"""
请根据下面的结构化网络告警数据生成企业微信摘要。**禁止自由发挥，严格按规则输出。**

格式和规则：
- 严格四段输出：**总体情况** / **未处理（重点）** / **已结束** / **处理中**
- **总体情况**：逐行列出窗口时间、未处理x条、已结束x条、处理中x条。
- **未处理（重点）**：数据中的 `alert_detail.未处理` 已列出该时段全部未处理告警明细。
  **你必须为数据中的每一条输出对应条目，不得遗漏、不得自行合并不同告警。**
  同一IP设备的多条告警可合并为一行，列出所有涉及的接口及对应故障，如"Ethernet1/17链路故障、Ethernet1/35连接状态down"。不同IP的告警绝对不可合并。
  每条格式：IP / 主机 / 主要故障内容（主机名称已含负责人，不再单独展示）。
  不展示具体告警时间（窗口时间已说明发生时段）。
- **已结束**：同上格式。同一IP设备合并为一行。
...
- **IP、设备名、接口等关键信息必须完整展示，不得截断、缩写或用省略号**。
- 精简原则：告警说明保留故障类型、设备、接口等关键信息。不展示"受影响网段""影响范围"等信息。
